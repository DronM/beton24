-- Function: vehicles_at_destination(timestamp without time zone, timestamp without time zone, integer, integer, interval)

-- DROP FUNCTION vehicles_at_destination(timestamp without time zone, timestamp without time zone, integer, integer, interval);

CREATE OR REPLACE FUNCTION vehicles_at_destination(IN in_date_time_from timestamp without time zone, IN in_date_time_to timestamp without time zone, IN in_destination_id integer, IN in_vehicle_id integer, IN in_stop_dur interval)
  RETURNS TABLE(vehicle_id integer, plate text, driver_name text, date_time text, stop_dur text) AS
$BODY$
BEGIN
	RETURN QUERY
	WITH dest_zone AS
...
$BODY$
  ...
  COST 100
  ROWS 1000;
ALTER FUNCTION vehicles_at_destination(timestamp without time zone, timestamp without time zone, integer, integer, interval)
  OWNER TO beton;
