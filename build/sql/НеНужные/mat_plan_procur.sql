...
'2014-12-05 06:59'::timestamp,
(SELECT get_shift_start(now()::timestamp-(const_days_for_plan_procur_val()||' days')::interval))::timestamp,
(SELECT get_shift_end((SELECT get_shift_start(now()::timestamp without time zone) - '1 day'::interval))),

7);
*/
/*Function: mat_plan_procur(
...
*/
/*
ДАТА НАЧАЛА ВСЕГДА ТЕКУЩАЯ СМЕНА+1
!!!РАБОТАЕТ ТОЛЬКО НА БУДУЩИЕ ДАТЫ!!!
*/
CREATE OR REPLACE FUNCTION mat_plan_procur(
...
$BODY$
  ...
  COST 100
  ROWS 1000;
ALTER FUNCTION mat_plan_procur(
	timestamp without time zone,
	timestamp without time zone,
	timestamp without time zone,
	integer
