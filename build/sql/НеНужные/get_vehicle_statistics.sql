-- Function: get_vehicle_statistics(date)

-- DROP FUNCTION get_vehicle_statistics(date);

CREATE OR REPLACE FUNCTION get_vehicle_statistics(IN in_date date)
  RETURNS TABLE(key text, val text) AS
$BODY$
DECLARE
...
BEGIN
	...
	*/
	...
	RETURN QUERY
		...
		SELECT
		'Коэффициент среднего на базе к рабочему времени ' || time5_descr(v_at_base_avg::time)::text || '/' || time5_descr(v_work_avg::time)::text AS key,
		(
		...
		SELECT
		'Коэффициент рабочего времени ко всему времени ' || time5_descr(v_work_avg::time)::text || '/' || time5_descr(v_end_time_interval::time)::text AS key,
		(
...
$BODY$
  ...
  COST 100
  ROWS 1000;
ALTER FUNCTION get_vehicle_statistics(date)
  OWNER TO beton;
