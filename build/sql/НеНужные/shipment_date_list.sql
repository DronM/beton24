-- View: shipment_date_list

--DROP VIEW shipment_date_list;

CREATE OR REPLACE VIEW shipment_date_list AS 
	SELECT 
		...
		time5_descr(SUM(sh.demurrage)::time) AS demurrage,
		SUM(calc_demurrage_coast(sh.demurrage)) AS demurrage_cost
	...
	GROUP BY 
		...
		date8_time5_descr(sh.ship_date_time::date),
		concr.id,
		concr.name,
		dest.id,
		dest.name,
...
ALTER TABLE shipment_date_list
  OWNER TO beton;
