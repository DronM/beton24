-- Function: available_spots_for_order(date, numeric, numeric)

-- DROP FUNCTION available_spots_for_order(date, numeric, numeric);

CREATE OR REPLACE FUNCTION available_spots_for_order(IN in_date date, IN in_quant numeric, IN in_speed_per_hour numeric)
  RETURNS TABLE(avail_date_time timestampTZ
...
DECLARE
	...
	record_fit boolean;
BEGIN
	order_min_step = constant_order_step_min();
	shift_start = in_date + constant_first_shift_start_time();
	shift_end = shift_start + constant_shift_for_orders_length_time() - '00:00:01'::time;
	norm_on_step = constant_max_hour_load()::numeric / 60 * order_min_step;
	tolerance = constant_order_auto_place_tolerance()/60 * order_min_step;

	CREATE TEMP TABLE IF NOT EXISTS shift_orders_with_spread_quant
	(date_time timestampTZ, quant_ordered numeric)
	ON COMMIT DROP;
...
in_quant, in_speed_per_hour, order_min_step));
	...
	OPEN shift_orders_curs SCROLL FOR SELECT
		...
		WHERE ( (in_date=CURRENT_DATE) AND (shift_orders_with_spread_quant.date_time >= round_minutes(CURRENT_TIMESTAMP::timestampTZ,order_min_step)) )
...
$BODY$
  ...
  COST 100
  ROWS 1000;
ALTER FUNCTION available_spots_for_order(date, numeric, numeric)
  OWNER TO beton;
