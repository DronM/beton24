...
CREATE OR REPLACE FUNCTION available_spots_for_order_dif_speed(IN in_date date, IN in_quant numeric, IN in_speed_per_hour numeric)
  RETURNS TABLE(avail_date_time timestampTZ, 
  --avail_time_descr text,
  avail_speed numeric) AS
$BODY$
DECLARE
	speed_tolerance int;
	i int;
BEGIN
	...
	(date_time timestampTZ, speed numeric ,CONSTRAINT res_table_pkey PRIMARY KEY (date_time))
	ON COMMIT DROP;
