-- Function: shipments_for_operator(timestamp without time zone, timestamp without time zone)

-- DROP FUNCTION shipments_for_operator(timestamp without time zone, timestamp without time zone);

CREATE OR REPLACE FUNCTION shipments_for_operator(IN in_date_time_from timestamp without time zone, IN in_date_time_to timestamp without time zone)
  RETURNS TABLE(sys_row boolean, quant_shipped numeric, quant_ordered numeric, id integer, client_descr character varying, destination_descr character varying, concrete_type_descr character varying, vehicle_descr text, driver_descr text, time_descr character varying, ship_time_descr character varying, date_time timestamp without time zone, date date, quant double precision, shipped boolean) AS
$BODY$
	SELECT * FROM (
		 (SELECT
		 true AS sys_row,
...
$BODY$
  ...
  COST 100
  ROWS 1000;
ALTER FUNCTION shipments_for_operator(timestamp without time zone, timestamp without time zone)
  OWNER TO beton;
