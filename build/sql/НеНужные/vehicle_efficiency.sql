...
CREATE OR REPLACE FUNCTION vehicle_efficiency(IN in_date_time_from timestamp without time zone, IN in_date_time_to timestamp without time zone, IN in_vehicle_id integer, IN in_vehicle_owner character varying, IN in_vehicle_feature character varying, IN in_run_type integer, IN in_shift_type integer)
...
BEGIN
	...
	LEFT JOIN (
		...
		SELECT
			...
			vehicles.owner,
			vehicles.feature,
			(SELECT d1 FROM get_shift_bounds(vehicle_schedules.schedule_date::timestamp without time zone) AS (d1 timestamp,d2 timestamp)) AS shift_start,	
			0 AS shipped_quant,
			0 AS runs,
			0 AS runs_day,
			0 AS runs_night,
