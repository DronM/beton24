...
CREATE OR REPLACE FUNCTION available_spots_for_order(IN in_date date, IN in_quant numeric, IN in_speed_per_hour numeric)
  RETURNS TABLE(avail_date_time timestamp without time zone, avail_time_descr text) AS
$BODY$
DECLARE
	o orders%rowtype;
	order_min_step int;
...
BEGIN
	...
	CREATE TEMP TABLE shift_orders_with_spread_quant
	...
	LOOP
		...
		WHILE quant_spread_ar_ind <= quant_spread_ar_len LOOP
			...
			IF  FOUND=false THEN
				...
				EXIT;
			END IF;
--RAISE EXCEPTION '%',shift_orders_r.rest;			
